...
# --- LOGIQUE DE CALCUL (TES RÈGLES COMPLETES) ---
def calculer_tour(rj, ra):
    ...
    # Détail des dés initiaux (le détail qu'il manquait)
...
async def lancer_match(ctx, names, is_solo=True, j2_obj=None):
    scores = {ctx.author.id: 0, ("CPU" if is_solo else j2_obj.id): 0}
    ...
    for tour in range(1, 8):
        ...
        if tour <= 6:
            ...
            scores[win_id] += 50
            emb_v = discord.Embed(title="🟡 VIF D'OR", description=f"🎲 {names[ctx.author.id]}: {v1} | {names[j2_id]}: {v2}\n🏆 **{names[win_id]}** l'attrape !", color=0xffd700)
            await ctx.send(emb_v)
...
async def match(ctx):
    ...
    try:
        reaction, user = await bot.wait_for('reaction_add', timeout=30, check=check_mode)
    ...
    msg_nom = await bot.wait_for('message', check=check_name)
    nom1 = msg_nom.content

    if mode == "solo":
        await lancer_match(ctx, {ctx.author.id: nom1, "CPU": "Équipe Adverse"}, is_solo=True)
    else:
        await ctx.send("🤝 Adversaire, tapez votre nom pour rejoindre !")

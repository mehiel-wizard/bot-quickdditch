...
app = Flask('')
@app.route('/')
def home(): return "L'arbitre de Quickdditch est sur le terrain ! 🏟️" [cite: 1]

def run(): app.run(host='0.0.0.0', port=8080) [cite: 2]
def keep_alive():
    t = Thread(target=run)
    t.start() [cite: 2]
...
bot = commands.Bot(command_prefix="!", intents=intents, help_command=None) [cite: 1]

def d(faces): return random.randint(1, faces) [cite: 2]
...
class VifDorMatchView(discord.ui.View):
    def __init__(self, game_instance, channel):
        ...
        self.lancers = {self.game.j1.id: None, self.game.j2_id: None} [cite: 8]
    ...
    async def lancer_vif(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        self.lancers[uid] = d(100) [cite: 22]
        
        if self.game.is_solo:
            self.lancers["CPU"] = d(100) [cite: 10]
        ...
        if all(v is not None for v in self.lancers.values()):
            ...
            n1, n2 = self.game.names[self.game.j1.id], self.game.names[self.game.j2_id] [cite: 11]
            
            win_id = self.game.j1.id if v1 > v2 else self.game.j2_id
            self.game.scores[win_id] += 50 [cite: 21, 22]
...
class NameModal(discord.ui.Modal):
    def __init__(self, player_num, parent_view):
        ...
        self.add_item(self.name_input) [cite: 2]

    async def on_submit(self, interaction: discord.Interaction):
        p_id = interaction.user.id
        self.parent_view.names[p_id] = self.name_input.value [cite: 3]
        await interaction.response.send_message(f"✅ Nom enregistré : **{self.name_input.value}**", ephemeral=True)
        await self.parent_view.check_start(interaction.channel) [cite: 3]
...
class StartMenuView(discord.ui.View):
    ...
    async def solo(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        view = SetupMatchView(self.author, is_solo=True) [cite: 4]
        await interaction.response.edit_message(content="🧙‍♂️ **Mode Solo** ! Choisissez votre nom :", view=view) [cite: 4, 5]
    ...
    async def duel(self, interaction: discord.Interaction, button: discord.ui.Button):
        if interaction.user != self.author: return
        await interaction.response.edit_message(content="🤝 **Duel** : Tapez `!duel @nom` pour défier quelqu'un !", view=None) [cite: 5]
...
class SetupMatchView(discord.ui.View):
    def __init__(self, j1, j2=None, is_solo=False):
        ...
        self.is_solo = is_solo [cite: 6]
        self.names = {j1.id: None, self.j2_id: "Équipe adverse" if is_solo else None} [cite: 6]
    ...
    async def set_j1(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        await interaction.response.send_modal(NameModal(1, self)) [cite: 6]
    ...
    async def set_j2(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        await interaction.response.send_modal(NameModal(2, self)) [cite: 7]
    async def check_start(self, channel):
        if all(v is not None for v in self.names.values()):
            self.stop()
            game = MatchView(self.j1, self.j2_id, self.names, self.is_solo) [cite: 7]
            await game.lancer_tour(channel) [cite: 7]

class MatchView(discord.ui.View):
    def __init__(self, j1, j2_id, names, is_solo):
        super().__init__(timeout=300) [cite: 8]
        self.j1, self.j2_id, self.names, self.is_solo = j1, j2_id, names, is_solo
        self.scores = {j1.id: 0, j2_id: 0} [cite: 8]
        self.tour = 1
        self.actions = {}

    async def lancer_tour(self, channel):
        self.actions = {self.j1.id: None} [cite: 8]
        if not self.is_solo: self.actions[self.j2_id] = None
        embed = discord.Embed(title=f"🏟️ TOUR {self.tour} / 6", color=discord.Color.blue()) [cite: 8, 9]
        embed.description = f"**{self.names[self.j1.id]}** ⚔️ **{self.names[self.j2_id]}**\n\nAttente des lancers..."
        await channel.send(embed=embed, view=self) [cite: 9]
    ...
    async def lancer_bouton(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        self.actions[uid] = {"atk": d(10), "def": d(6), "bat": d(4)} [cite: 9]
        if self.is_solo: self.actions["CPU"] = {"atk": d(10), "def": d(6), "bat": d(4)} [cite: 10]
        if all(v is not None for v in self.actions.values()):
            await interaction.response.edit_message(content="🎲 **Calcul des résultats...**", embed=None, view=None) [cite: 10]
            await self.resolution_tour(interaction.channel) [cite: 10]
        else:
            lances = [self.names[u] for u, a in self.actions.items() if a is not None]
            status = "\n".join([f"✅ **{n}** a lancé ses dés !" for n in lances]) [cite: 11]
            embed = interaction.message.embeds[0]
            embed.description = f"**{self.names[self.j1.id]}** ⚔️ **{self.names[self.j2_id]}**\n\n{status}" [cite: 11]
            await interaction.response.edit_message(embed=embed, view=self) [cite: 11]

    async def resolution_tour(self, channel):
        n1, n2 = self.names[self.j1.id], self.names[self.j2_id]
        r1, r2 = self.actions[self.j1.id], self.actions[self.j2_id] [cite: 11]
        def calculer(r_j, r_adv):
            nb_buts, b_v = 0, r_j['bat']
            txt_b, b_a, b_d = f"🏏 **Batteur ({b_v})** : ", 0, 0
            s_a, s_d = str(r_j['atk']), str(r_j['def'])
            if b_v == 1: txt_b += "⚠️ **Faute ! (-2 Déf)**"; b_d = -2; s_d = f"{r_j['def']}-2" [cite: 12, 13]
            elif b_v == 2: txt_b += "🛡️ **Renfort ! (+2 Déf)**"; b_d = 2; s_d = f"{r_j['def']}+2" [cite: 13, 14]
            elif b_v == 3: txt_b += "🎯 **Ouverture ! (+2 Atk)**"; b_a = 2; s_a = f"{r_j['atk']}+2" [cite: 14, 15]
            elif b_v == 4: txt_b += "💥 **Exploit ! (+1 but)**" [cite: 15, 16]
            b_da = -2 if r_adv['bat'] == 1 else (2 if r_adv['bat'] == 2 else 0) [cite: 16]
            s_da = f"{r_adv['def']}{'+' if b_da > 0 else ''}{b_da}" if b_da != 0 else str(r_adv['def']) [cite: 16]
            f_a, f_da = r_j['atk'] + b_a, r_adv['def'] + b_da [cite: 16]
            ecart = f_a - f_da [cite: 17]
            txt_a = f"\n🏹 **Attaque ({s_a})** vs **Défense ({s_da})** : " [cite: 17]
            if ecart > 0:
                nb_buts = 3 if ecart >= 8 else (2 if ecart > 3 else 1) [cite: 17, 18]
                txt_a += f"✅ **Réussi ! (Écart: {ecart})**" [cite: 18, 19]
            else: txt_a += "🧤 **Arrêté !**" [cite: 19]
            if b_v == 4: nb_buts += 1 [cite: 19]
            pts = nb_buts * 10 [cite: 19]
            return pts, f"{txt_b}{txt_a}\n➡️ **RÉSULTAT : {nb_buts} but(s) ({pts} pts)**" [cite: 19]

        p1, d1 = calculer(r1, r2)
        p2, d2 = calculer(r2, r1) [cite: 19]
        self.scores[self.j1.id] += p1
        self.scores[self.j2_id] += p2 [cite: 20]
        embed = discord.Embed(title=f"⚖️ RÉSULTATS TOUR {self.tour}", color=discord.Color.gold()) [cite: 20]
        embed.add_field(name=f"🧤 {n1}", value=d1, inline=False)
        embed.add_field(name=f"🧤 {n2}", value=d2, inline=False)
        embed.set_footer(text=f"Score : {n1} {self.scores[self.j1.id]} - {self.scores[self.j2_id]} {n2}") [cite: 20]
        await channel.send(embed=embed)
        self.tour += 1
        if self.tour <= 6: await asyncio.sleep(1); await self.lancer_tour(channel) [cite: 20, 21]
        else: await self.vif_dor(channel) [cite: 21]

    async def vif_dor(self, channel):
        await channel.send("\n✨ **7ème TOUR : VIF D'OR !**") [cite: 21]
        await asyncio.sleep(2)
        # On lance le duel manuel de d100
...
@bot.command()
async def match(ctx):
    await ctx.send("🧙‍♂️ **Bienvenue à une partie de Quickdditch !**", view=StartMenuView(ctx.author)) [cite: 23]

@bot.command()
async def duel(ctx, adversaire: discord.Member):
    if adversaire.bot or adversaire == ctx.author: return [cite: 22]
    view = SetupMatchView(ctx.author, adversaire) [cite: 22]
    await ctx.send(f"🏟️ **Duel lancé !**\n{ctx.author.mention} et {adversaire.mention}, à vos balais !", view=view) [cite: 23]
...
keep_alive() [cite: 23]
bot.run(os.environ['DISCORD_TOKEN']) [cite: 23]

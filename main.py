...
# --- SERVEUR WEB ANTI-BLOCAGE ---
app = Flask('')
@app.route('/')
def home(): return "Arbitre en ligne ! 🏟️"

def run():
    # use_reloader=False empêche Flask de lancer deux instances du bot
...
class MatchView(discord.ui.View):
    ...
    async def lancer_bouton(self, interaction: discord.Interaction, button: discord.ui.Button):
        # 1. RÉPONSE IMMÉDIATE (Plus d'échec d'interaction)
        ...
        async with self.lock:
            ...
            if all(v is not None for v in self.actions.values()) and not self._calcul_lance:
                self._calcul_lance = True
                donnees = self.actions.copy()
                self.actions.clear()
                self.stop()
    ...
    async def resolution_tour(self, channel, acts):
        ...
        def calculer(r_j, r_adv):
            ...
            if b_v == 1: 
                txt_b += "⚠️ **Faute ! (-2 Déf)**"; b_d = -2; s_d = f"{r_j['def']}-2"
            elif b_v == 2: 
                txt_b += "🛡️ **Renfort ! (+2 Déf)**"; b_d = 2; s_d = f"{r_j['def']}+2"
            elif b_v == 3: 
                txt_b += "🎯 **Ouverture ! (+2 Atk)**"; b_a = 2; s_a = f"{r_j['atk']}+2"
            elif b_v == 4: 
                txt_b += "💥 **Exploit ! (+1 but)**"
...
@bot.event
async def on_ready(): print(f"✅ Arbitre opérationnel !")

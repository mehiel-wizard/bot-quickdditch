    ...
    async def lancer_vif(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if all(v is not None for v in self.lancers.values()):
            ...
            embed = discord.Embed(title="🟡 RÉSULTAT DU VIF D'OR", 
                                  description=f"🎲 **{n1}** : `{v1}` |import discord
from discord.ext import commands
import os
import random
import asyncio
...
app = Flask('')
@app.route('/')
def home(): return "L'arbitre de Quickdditch est là ! 🏟️"
...
class VifDorMatchView(discord.ui.View):
    ...
    async def lancer_vif(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if all(v is not None for v in self.lancers.values()):
            ...
            await interaction.response.edit_message(content="✨ Capture terminée !", view=None)
...
class MatchView(discord.ui.View):
    ...
    async def lancer_bouton(self, interaction: discord.Interaction, button: discord.ui.Button):
        uid = interaction.user.id
        if uid not in self.actions or self.actions[uid] is not None or self.processing: return
    ...
    async def vif_dor(self, channel):
        await channel.send("\n✨ **7ème TOUR : LE VIF D'OR EST APPARU !**")
        view = VifDorMatchView(self, channel)
        await channel.send(f"🏆 Cliquez sur le bouton doré pour votre duel final !", view=view)

# --- RESTE DU CODE (NameModal, Setup, etc.) ---
...
# --- COMMANDES ---
@bot.command()
async def match(ctx): await ctx.send("🧙‍♂️ Bienvenue !", view=StartMenuView(ctx.author))
@bot.command()
async def helpquickdditch(ctx): await ctx.send("🏟️ `!match` | `!reglesquickdditch`")
@bot.command()
async def reglesquickdditch(ctx): await ctx.send("📜 6 tours + Vif d'Or (+50 pts) !")
@bot.command()
